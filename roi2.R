...
setkey(ph,PERIOD_END_LSTAT,PBAL_BEG_PERIOD,id)
...
ids=as.data.table(unique(stats$id))
names(ids) <- 'id'
ph <- merge(x = ph, y = ids, by = "id")
...
# Get cash flows
cashOut <- ph[, .(Month=min(Month),Payment=-max(Principal)), keyby = id]
cashIn <- ph[,.(Month,Payment=Payment*.99)]

cashFlow <- rbind(cashIn,cashOut[,.(Month,Payment)])

j <- cashFlow[, .(Payment=sum(Payment)), keyby = Month]
xIRR(j)
...
cashFlow <- ph[,.(Month = dmy(paste("01", MONTH , sep ="")),Payment = RECEIVED_AMT)]
...
# ph$MONTH <- dmy(paste("01", ph$MONTH , sep =""))
# p <- ifelse(is.na(ph$RECEIVED_D),ph$MONTH,ph$RECEIVED_D)
