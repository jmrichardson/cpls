...
for (i in 1:length(users)) {
  ...
  users[[i]]$html <- gsub('PROCTIME',paste (users[[i]]$elapsedProcTime,'s'),users[[i]]$html,sep='')
  users[[i]]$html <- gsub('ORDERTIME',paste(users[[i]]$elapsedOrderTime,'s'),users[[i]]$html,sep='')
  users[[i]]$html <- gsub('TOTALTIME',paste (users[[i]]$elapsedTotalTime,'s'),users[[i]]$html,sep='')
  ...
  users[[i]]$html <- gsub('DOWNTIME',paste(round(apiTimeElapse,2),"s"),users[[i]]$html,sep='')
